...
export const sessionStatusSchema = z.enum(["draft", "steady", "scaling", "degraded", "terminated"]);
...
const hypergridSampleSchema = z.object({
  sessionId: z.string(),
  label: z.string(),
  status: sessionStatusSchema,
  url: z.string(),
  activeReplicas: z.number().int().nonnegative(),
  averageLatencyMs: z.number().nonnegative()
});

export const hypergridTileSchema = z.object({
  tileId: z.string(),
  coordinate: z.tuple([z.number().int().nonnegative(), z.number().int().nonnegative()]),
  sessionCount: z.number().int().nonnegative(),
  statusHistogram: z.record(z.string(), z.number().int().nonnegative()),
  averageLatencyMs: z.number().nonnegative(),
  activeReplicas: z.number().int().nonnegative(),
  proxyCountries: z.record(z.string(), z.number().int().nonnegative()),
  density: z.number().nonnegative(),
  samples: z.array(hypergridSampleSchema).max(12)
});

export const hypergridSnapshotSchema = z.object({
  generatedAt: z.number().int(),
  gridSize: z.number().int().positive(),
  totalSessions: z.number().int().nonnegative(),
  maxSessionsPerTile: z.number().int().nonnegative(),
  tiles: z.array(hypergridTileSchema),
...
export type HypergridTile = z.infer<typeof hypergridTileSchema>;
export type HypergridSnapshot = z.infer<typeof hypergridSnapshotSchema>;
